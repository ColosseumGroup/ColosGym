...
static int consumeSpaces(const char *string, int consumeEqual)
{
  ...
  for (i = 0; string[i] != 0 && (isspace(string[i]) || (consumeEqual && string[i] == '='));
       ++i)
  {
  }
...
Game *readGame(FILE *file)
{
  ...
  if (game->bettingType == limitBetting && raiseSizeRead < game->numRounds)
  {
  ...
  for (c = 0; c < game->numRounds; ++c)
  {
    ...
    if (game->firstPlayer[c] == 0 || game->firstPlayer[c] > game->numPlayers)
    {
...
uint8_t numActingPlayers(const Game *game, const State *state)
{
  ...
  for (p = 0; p < game->numPlayers; ++p)
  {
    if (state->playerFolded[p] == 0 && state->spent[p] < game->stack[p])
    {
      ++ret;
    }
  }
                          ...
                          State *state)
{
  ...
  for (i = 0; i <= state->round; ++i)
  {
    ...
    if (readCards(&string[c], game->numBoardCards[i],
                  &state->boardCards[bcStart(game, i)], &r) != game->numBoardCards[i])
    {
